...
from video2blog.keyframe_extractor import extract_keyframes, detect_keyframe_timestamps
from video2blog import db_service
from video2blog import gemini_service
from video2blog.editing_routes import editing_bp
...
def _resolve_gemini_uri(filename: str, filepath: str | None = None) -> tuple[str, str]:
    ...
    if not filepath or not os.path.exists(filepath):
        raise FileNotFoundError(
            f"Local video file not found for re-upload: {filepath!r}"
        )
    ...
    if os.path.exists(app.config["UPLOAD_FOLDER"]):
        for filename in sorted(os.listdir(app.config["UPLOAD_FOLDER"])):
            ...
            entry = {
                ...
                "gemini_last_verified_at": gemini_info.get("last_verified_at")
                if gemini_info
                else None,
                "can_select": True,
...
def extract_frames_url():
    ...
    try:
        ...
        if timestamps_raw:
            timestamps = [
                float(t.strip()) for t in str(timestamps_raw).split(",") if t.strip()
            ]
...
def extract():
    ...
    try:
        strategy = "timestamp" if mode == "timestamp" else "scene"
        timestamps = None
        if strategy == "timestamp":
            ...
            try:
                ...
                return jsonify(
                    {"error": "Invalid timestamp format. Use comma-separated numbers"}
                ), 400
        ...
        results = extract_keyframes(
            ...
            strategy=strategy,
            threshold=threshold,
            frames_per_interval=frames_per_point,
            timestamps=timestamps,
...
def download_single_frame(job_name, filename):
    ...
    return send_file(
        filepath, mimetype="image/jpeg", as_attachment=True, download_name=filename
    )
    ...
    if os.path.exists(app.config["OUTPUT_FOLDER"]):
        for item in os.listdir(app.config["OUTPUT_FOLDER"]):
            ...
            if os.path.isdir(item_path):
                frame_count = len(
                    [f for f in os.listdir(item_path) if f.endswith(".jpg")]
                )
                jobs.append(
                    {"name": item, "frame_count": frame_count, "path": item_path}
                )
...
def delete_all_frames_api(job_name):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": f"Deleted all frames for {job_name}"}
        )
    except Exception as e:
...
def gemini_generate_blog():
    ...
    try:
        strategy = "timestamp" if timestamps else "scene"
        ts_list = [float(t) for t in timestamps] if timestamps else None
...
def get_connections():
    ...
    return jsonify({"connections": connections, "active_id": active_id})
