...
def stream_edit():
    ...
    if not editing_service.is_configured():
        return jsonify(
            {"error": "Editing service not configured. Set EDITING_API_KEY."}
        ), 503
    ...
    if system_prompt_override:
        ...
        if mode_name:
            try:
                ...
                mode_label = next(
                    (
                        m.get("label", mode_name)
                        for m in (skill_data.get("modes") or [])
        ...
        for param in merged_params:
            if (
                param.get("required")
                and not str(parameters.get(param["name"], "")).strip()
            ):
                return jsonify(
                    {"error": f"Required parameter missing: {param['label']}"}
                ), 400

    transcript = transcript_override or draft.get("transcript")

    def generate():
        yield from editing_service.stream_edit(
            system_prompt, draft["content"], transcript, parameters=parameters or None
        )
...
@editing_bp.route(
    "/editing/drafts/<int:draft_id>/restore/<int:version_id>", methods=["POST"]
)
